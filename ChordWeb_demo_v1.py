...
# ---------- Audio ----------
def synthesize_chord_wav_bytes(notes, seconds=0.9, sr=44100, gain=0.6, soft_k=0.0):
    if not notes:
        return b"", sr
    base_midi = 48
    ...
    # Optional soft clip to tame peaks
    if soft_k and soft_k > 0:
        wave_f = np.tanh(soft_k * wave_f) / np.tanh(soft_k)
    # Apply volume headroom (0..1)
...
def play_with_afplay_or_simpleaudio(notes, gain=0.6):
    data, sr = synthesize_chord_wav_bytes(notes, gain=gain)
    if not data:
        return
...
class ChordApp(tk.Tk):
    def __init__(self):
        ...
        # Diatonic slider feature
        self.rows_enabled = tk.BooleanVar(value=False)
        self.rows_level   = tk.IntVar(value=1)
        # Output volume (0-100)
        self.vol_var = tk.DoubleVar(value=60.0)
...
# Controls (horizontal)
        ...
        rows_row = ttk.Frame(top); rows_row.pack(side=tk.TOP, fill=tk.X, pady=(6,2))
        ttk.Label(rows_row, text="Rows", width=10).pack(side=tk.LEFT, padx=(0,6))
        ttk.Checkbutton(rows_row, variable=self.rows_enabled).pack(side=tk.LEFT, padx=(0,12))
        ttk.Label(rows_row, text="Diatonic  1  ↔  10  Chromatic").pack(side=tk.LEFT, padx=(0,6))
        self.rows_slider = tk.Scale(rows_row, from_=1, to=10, orient="horizontal", variable=self.rows_level, resolution=1, tickinterval=1, showvalue=1, length=320)
        self.rows_slider.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(6,6))
        ...
        ttk.Label(count_row, text="Vol", width=6).pack(side=tk.LEFT, padx=(14,0))
        self.vol_slider = tk.Scale(count_row, from_=0, to=100, orient="horizontal", variable=self.vol_var, resolution=1, tickinterval=25, showvalue=1, length=200)
        self.vol_slider.pack(side=tk.LEFT, padx=(6,6))
...
# === Starting chord feature helpers ===
    ...
    def collect_buckets(self, key, base, sub):
        """Return a list of rows (each row is a list of chord strings) for the current selection."""
        buckets = []
        keys = self.sheet_names if key == "All Keys" else [key]
        for k in keys:
            bdict = self.structured_data.get(k, {}) or {}
            bases = list(bdict.keys()) if base == "All Base Modes" else [base]
            for b in bases:
                sdict = bdict.get(b, {}) or {}
                subs = list(sdict.keys()) if sub == "All Sub Modes" else [sub]
                for s in subs:
                    rows = sdict.get(s, []) or []
                    for r in rows:
                        rr = [c for c in r if c]
                        if rr:
                            buckets.append(rr)
        return buckets
    ...
    def generate_chords(self):
        ...
        buckets = self.collect_buckets(key, base, sub)
        if not pool:
            self.clear_results()
        ...
        start = start if start not in (None, "", "(none)") else None

        # History
        self.push_history()

        # --- Rows/diatonic logic ---
        if self.rows_enabled.get() and buckets:
            # Determine the "home" row
            home_idx = None
            if start:
                for i, r in enumerate(buckets):
                    if start in r:
                        home_idx = i; break
            if home_idx is None:
                # Use the longest row as home
                home_idx = max(range(len(buckets)), key=lambda i: len(buckets[i])) if buckets else 0
            home = buckets[home_idx] if buckets else []

            level = int(self.rows_level.get() or 1)   # 1 = strict, 10 = many outsiders
            outsiders_allowed = max(0, level - 1)

            seq = []
            if start and start in pool:
                seq.append(start)
            ...
            while len(seq) < num:
                ...
                # Prefer home
                pick = None
                pool_home = [c for c in home if c not in seq]
                if pool_home:
                    pick = random.choice(pool_home)
                # Allow occasional outsider
                if (pick is None or random.random() < (outsiders_allowed / 12.0)) and outsiders_used < outsiders_allowed:
                    outside = [c for i, r in enumerate(buckets) for c in r if i != home_idx and c not in seq]
                    if outside:
                        pick = random.choice(outside)
                        outsiders_used += 1
                # Fallback
                if pick is None:
                    fallback = [c for c in pool if c not in seq]
                    if not fallback:
                        break
                    pick = random.choice(fallback)
                if last and pick == last:
                    continue
                seq.append(pick)
            sample = seq
        else:
            # --- Original behavior (flat sample) ---
            if start and start in pool:
    ...
    def on_keypress(self, event):
        # Normalize key from char or keysym; support numpad KP_1..KP_8
        ch = event.char if getattr(event, 'char', None) else ''
        ks = event.keysym if getattr(event, 'keysym', None) else ''
        if not ch and ks.startswith('KP_'):
            ch = ks.replace('KP_', '')
        if not ch and ks.isdigit():
            ch = ks
        if not ch:
            return
        ch = ch.lower()
    ...
    def play_index(self, index):
        ...
        root, qual = parsed
        notes, _ = build_7th_chord(root, qual)
        threading.Thread(target=play_with_afplay_or_simpleaudio, args=(notes, float(self.vol_var.get())/100.0), daemon=True).start()
